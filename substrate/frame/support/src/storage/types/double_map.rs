...
use crate::{
	storage::{
		...
		StoragePrefixedMap, StorageTryAppend,
	},
	...
	MaxValues: Get<Option<u32>>,
{
	...
	{
		Self::double_map_final_key(k1, k2)
	}
	...
	where
		...
		KArg2: EncodeLike<Key2>,
	{
		unhashed::exists(&Self::double_map_final_key(k1, k2))
	}
	...
	where
		...
		KArg2: EncodeLike<Key2>,
	{
		QueryKind::from_optional_value_to_query(unhashed::get(&Self::double_map_final_key(k1, k2)))
	}
	...
	where
		...
		KArg2: EncodeLike<Key2>,
	{
		unhashed::get(&Self::double_map_final_key(k1, k2)).ok_or(())
	}
	...
	where
		...
		KArg2: EncodeLike<Key2>,
	{
		let final_key = Self::double_map_final_key(k1, k2);
	...
	{
		let final_x_key = Self::double_map_final_key(x_k1, x_k2);
		let final_y_key = Self::double_map_final_key(y_k1, y_k2);
	...
	{
		unhashed::put(&Self::double_map_final_key(k1, k2), &val)
	}
	...
	where
		...
		KArg2: EncodeLike<Key2>,
	{
		unhashed::kill(&Self::double_map_final_key(k1, k2))
	}
	...
	where
		KArg1: EncodeLike<Key1>,
	{
		unhashed::clear_prefix(Self::double_map_prefix_key1(k1).as_ref(), maybe_limit, None).into()
	}
	...
	where
		KArg1: EncodeLike<Key1>,
	{
		unhashed::clear_prefix(Self::double_map_prefix_key1(k1).as_ref(), Some(limit), maybe_cursor)
			.into()
	}
	...
	where
		KArg1: EncodeLike<Key1>,
	{
		unhashed::contains_prefixed_key(Self::double_map_prefix_key1(k1).as_ref())
	}
	...
	{
		let prefix = Self::double_map_prefix_key1(k1);
	...
	>(
		...
		let old_key = {
			...
			let mut final_key = Vec::with_capacity(
				sp.len() + key1_hashed.as_ref().len() + key2_hashed.as_ref().len(),
			);
		...
		unhashed::take(old_key.as_ref()).inspect(|value| {
			unhashed::put(Self::double_map_final_key(key1, key2).as_ref(), &value);
		})
	}
}
...
where
	...
	MaxValues: Get<Option<u32>>,
{
	/// Generate the key prefix for a given first key (used for partial iteration/removal).
	fn double_map_prefix_key1<KArg1: EncodeLike<Key1>>(k1: KArg1) -> Vec<u8> {
		let sp =
			storage_prefix(Prefix::pallet_prefix().as_bytes(), Prefix::STORAGE_PREFIX.as_bytes());
		let key_hashed = k1.using_encoded(Hasher1::hash);
		let mut final_key = Vec::with_capacity(sp.len() + key_hashed.as_ref().len());
		final_key.extend_from_slice(&sp);
		final_key.extend_from_slice(key_hashed.as_ref());
		final_key
	}

	/// Generate the full key for a (k1, k2) pair.
	fn double_map_final_key<KArg1: EncodeLike<Key1>, KArg2: EncodeLike<Key2>>(
		k1: KArg1,
		k2: KArg2,
	) -> Vec<u8> {
		let sp =
		...
		let mut final_key =
			Vec::with_capacity(sp.len() + key1_hashed.as_ref().len() + key2_hashed.as_ref().len());
		final_key.extend_from_slice(&sp);
		final_key.extend_from_slice(key1_hashed.as_ref());
		final_key.extend_from_slice(key2_hashed.as_ref());
		final_key
