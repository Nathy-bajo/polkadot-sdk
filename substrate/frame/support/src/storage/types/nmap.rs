...
use crate::{
	storage::{
		...
		StorageDecodeLength, StoragePrefixedMap, StorageTryAppend,
	},
...
impl<Prefix, Key, Value, QueryKind, OnEmpty, MaxValues> storage::StorageNMap<Key, Value>
	...
	MaxValues: Get<Option<u32>>,
{
	...
	fn take<KArg: EncodeLikeTuple<Key::KArg> + TupleToEncodedIter>(key: KArg) -> Self::Query {
		let final_key = Self::n_map_final_key(key);
	...
	{
		let final_x_key = Self::n_map_final_key(key1);
		let sp =
			storage_prefix(Prefix::pallet_prefix().as_bytes(), Prefix::STORAGE_PREFIX.as_bytes());
		let key2_hashed = KOther::final_key(key2);
		let mut final_y_key = Vec::with_capacity(sp.len() + key2_hashed.len());
		final_y_key.extend_from_slice(&sp);
		final_y_key.extend_from_slice(key2_hashed.as_ref());
	...
	{
		unhashed::put(&Self::n_map_final_key(key), &val);
	}

	fn remove<KArg: EncodeLikeTuple<Key::KArg> + TupleToEncodedIter>(key: KArg) {
		unhashed::kill(&Self::n_map_final_key(key));
	}
	...
	where
		Key: HasKeyPrefix<KP>,
	{
		unhashed::clear_prefix(&Self::n_map_partial_key(partial_key), limit, None).into()
	}
	...
	where
		Key: HasKeyPrefix<KP>,
	{
		unhashed::clear_prefix(&Self::n_map_partial_key(partial_key), Some(limit), maybe_cursor)
	}
	...
	where
		Key: HasKeyPrefix<KP>,
	{
		unhashed::contains_prefixed_key(&Self::n_map_partial_key(partial_key))
	}
	...
	where
		Key: HasKeyPrefix<KP>,
	{
		let prefix = Self::n_map_partial_key(partial_key);
		PrefixIterator {
			prefix: prefix.clone(),
			previous_key: prefix,
			drain: false,
	...
	{
		let final_key = Self::n_map_final_key(key);
		let mut val = QueryKind::from_optional_value_to_query(unhashed::get(final_key.as_ref()));
		let ret = f(&mut val);
		if ret.is_ok() {
	...
	{
		let old_key = {
			let sp = storage_prefix(
				Prefix::pallet_prefix().as_bytes(),
				Prefix::STORAGE_PREFIX.as_bytes(),
			);
			let key_hashed = Key::migrate_key(&key, hash_fns);
			let mut final_key = Vec::with_capacity(sp.len() + key_hashed.len());
			final_key.extend_from_slice(&sp);
			final_key.extend_from_slice(key_hashed.as_ref());
			final_key
		};
		unhashed::take(old_key.as_ref()).inspect(|value| {
			unhashed::put(Self::n_map_final_key(key).as_ref(), &value);
		})
	}
}
	...
	MaxValues: Get<Option<u32>>,
{
	...
	{
		let prefix = Self::n_map_partial_key(kp);
		PrefixIterator {
			prefix: prefix.clone(),
			previous_key: prefix,
			drain: false,
	...
	fn iter() -> Self::Iterator {
		Self::iter_from(Prefix::prefix_hash().to_vec())
	}

	fn iter_from(starting_raw_key: Vec<u8>) -> Self::Iterator {
		let prefix = Prefix::prefix_hash().to_vec();
		Self::Iterator {
			prefix,
	...
	fn iter_keys() -> Self::KeyIterator {
		Self::iter_keys_from(Prefix::prefix_hash().to_vec())
	}
...
where
	...
	MaxValues: Get<Option<u32>>,
{
	/// Generate the final storage key for a complete key tuple.
	fn n_map_final_key<KArg: EncodeLikeTuple<Key::KArg> + TupleToEncodedIter>(
		key: KArg,
	) -> Vec<u8> {
		let sp =
			storage_prefix(Prefix::pallet_prefix().as_bytes(), Prefix::STORAGE_PREFIX.as_bytes());
		let key_hashed = Key::final_key(key);
		let mut final_key = Vec::with_capacity(sp.len() + key_hashed.len());
		final_key.extend_from_slice(&sp);
		final_key.extend_from_slice(key_hashed.as_ref());
		final_key
	...
	/// Generate a partial storage key for a prefix of the key tuple.
	fn n_map_partial_key<KP>(partial_key: KP) -> Vec<u8>
	where
		Key: HasKeyPrefix<KP>,
	{
		let sp =
			storage_prefix(Prefix::pallet_prefix().as_bytes(), Prefix::STORAGE_PREFIX.as_bytes());
		let key_hashed = <Key as HasKeyPrefix<KP>>::partial_key(partial_key);
		let mut final_key = Vec::with_capacity(sp.len() + key_hashed.len());
		final_key.extend_from_slice(&sp);
		final_key.extend_from_slice(key_hashed.as_ref());
		final_key
