...
impl<T: Config<I>, I: 'static> fungibles::Inspect<<T as SystemConfig>::AccountId> for Pallet<T, I> {
	...
	fn total_balance(asset: &Self::AssetId, who: &<T as SystemConfig>::AccountId) -> Self::Balance {
		Pallet::<T, I>::balance(asset.clone(), who)
			.saturating_add(T::Holder::balance_on_hold(asset.clone(), who).unwrap_or_default())
	}
	...
	fn reducible_balance(
		...
		Pallet::<T, I>::reducible_balance(asset.clone(), who, !matches!(preservation, Expendable))
			.unwrap_or(Zero::zero())
	}
	...
	fn can_deposit(
		...
		Pallet::<T, I>::can_increase(asset.clone(), who, amount, provenance == Minted)
	}
	...
	fn can_withdraw(
		...
		Pallet::<T, I>::can_decrease(asset.clone(), who, amount, false)
	}
	...
	for Pallet<T, I>
{
	...
	fn done_rescind(asset_id: &Self::AssetId, amount: Self::Balance) {
		Self::deposit_event(Event::IssuedDebt { asset_id: asset_id.clone(), amount })
	}

	fn done_issue(asset_id: &Self::AssetId, amount: Self::Balance) {
		Self::deposit_event(Event::IssuedCredit { asset_id: asset_id.clone(), amount })
	}
}
...
impl<T: Config<I>, I: 'static> fungibles::Unbalanced<T::AccountId> for Pallet<T, I> {
	...
	fn decrease_balance(
		...
		Self::decrease_balance(asset.clone(), who, amount, f, |_, _| Ok(()))
	}
	fn increase_balance(
		asset: &T::AssetId,
		who: &T::AccountId,
		...
		Self::increase_balance(asset.clone(), who, amount, |_| Ok(()))?;
		Ok(amount)
	}
	...
	for Pallet<T, I>
{
	fn set(
		...
		Self::do_set_metadata(asset.clone(), from, name, symbol, decimals)
	}
}
	...
	for Pallet<T, I>
{
	...
	fn approve(
		...
		Self::do_approve_transfer(asset.clone(), owner, delegate, amount)
	}
	...
	fn transfer_from(
		...
		Self::do_transfer_approved(asset.clone(), owner, delegate, dest, amount)
	}
}
