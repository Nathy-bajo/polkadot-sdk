...
fn reset_base_price_doesnt_affect_renewals_immediately() {
	TestExt::new().endow(1, 1000).endow(1001, 10000).execute_with(|| {
		assert_ok!(Broker::do_start_sales(100, 1));
		advance_to(2);
		...
		// Advance to when the renewal record should be created
		advance_sale_period();

		// Now check that a renewal record exists
		let sale = SaleInfo::<Test>::get().unwrap();
		let renewal_id = PotentialRenewalId { core: region.core, when: sale.region_begin };
		let initial_renewal_record = PotentialRenewals::<Test>::get(renewal_id).unwrap();

		// The renewal price should be 100 (the price from the first sale)
		assert_eq!(initial_renewal_record.price, 100);

		// Reset price during this sale period
		assert_ok!(Broker::reset_base_price(RuntimeOrigin::root(), 50));

		// The existing renewal record should NOT change
		let renewal_record_after_reset = PotentialRenewals::<Test>::get(renewal_id).unwrap();
		assert_eq!(
			renewal_record_after_reset.price, 100,
			"Existing renewal record price should not change when base price is reset"
		);

		// Now let's actually renew manually to verify the price
		let balance_before_renew = balance(1001);
		assert_ok!(Broker::do_renew(1001, region.core));
		let balance_after_renew = balance(1001);

		// The renewal should cost 100 (the old price), not 50 (the new base price)
		let renewal_cost = balance_before_renew - balance_after_renew;
		assert_eq!(
			renewal_cost, 100,
			"Renewal should use existing renewal record price, not new base price"
		);

		// After this renewal, a new renewal record is created for the next period
		// Advance to the next sale period where new base price takes effect
		advance_sale_period();

		// The new sale should have the new base price
		let new_sale = SaleInfo::<Test>::get().unwrap();
		assert_eq!(new_sale.end_price, 50, "New sale should have new base price");
	});
}
...
#[test]
fn reset_base_price_with_admin_origin_works() {
	TestExt::new().endow(1, 1000).execute_with(|| {
		assert_ok!(Broker::do_start_sales(100, 1));
		advance_to(2);

		assert_ok!(Broker::reset_base_price(RuntimeOrigin::signed(1), 75));
